...
def register_new_user(email, user, password):
    assigned_id = generate_user_id()
    if assigned_id in users:
        return register_new_user(email, user, password)
    users[assigned_id] = {"email": email, "user": user, "password": password}
    print(users)
    return assigned_id
...
# Obtener usuarios conectados desde el servidor
